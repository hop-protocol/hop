...
export async function getUSDCSwapParams(options) {
    ...
    if (getQuote) {
        if (BigNumber.from(amountIn).eq(0)) {
            ...
            const quoterContract = new Contract(quoter, UniswapQuoterV2Abi, provider);
            ...
            //   0
            // )
            quotedAmountOut = await quoterContract.callStatic.quoteExactInput(encodeRouteToPath(route, exactOutput), amountIn.toString());
