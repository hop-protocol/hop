...
export function GetMessageSentEventFromTxHash (props: Props) {
  const cacheKey = 'getMessageSentEventFromTxHash'
  const { sdk } = props
...
`.trim()
  ...
  return (
    <Box>
      <Box mb={1}>
        <Typography variant="h5">Messenger - Get Message Sent Event From Transaction Hash</Typography>
      </Box>
      <Box mb={4}>
        <Typography variant="subtitle1">Get full event log from transaction hash</Typography>
      </Box>
      <Box width="100%" display="flex" justifyContent="space-between" className={styles.container}>
        <Box mr={4} className={styles.formContainer}>
          <form onSubmit={handleSubmit}>
            ...
            <Box mb={2}>
              <Box mb={1}>
                <label>Transaction Hash<small><em>(hex string)</em></small> <small><em>This is the transaction hash of the origin message send</em></small></label>
              </Box>
...
export default GetMessageSentEventFromTxHash
