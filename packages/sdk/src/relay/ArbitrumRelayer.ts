...
export class ArbitrumRelayer {
  ...
  constructor (networkSlug: string, chainSlug: string, l1Wallet: Signer | providers.Provider, l2Wallet: Signer | providers.Provider) {
    this.l1Wallet = l1Wallet
    this.l2Wallet = l2Wallet as Signer
  }
  ...
  async #getL1ToL2Message (txHash: string, messageIndex: number): Promise<Message> {
    const txReceipt: providers.TransactionReceipt = await (this.l1Wallet as providers.Provider).getTransactionReceipt(txHash)
    if (!txReceipt) {
