...
export function RailsGatewayGetTransferBondedEventFromCheckpoint (props: Props) {
  const cacheKey = 'railsGatewayGetTransferBondedEventFromCheckpoint'
  const { sdk } = props
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const event = await sdk.railsGateway.getTransferBondedEventFromCheckpoint(args)
...
async function main() {
  ...
  const event = await hop.railsGateway.getTransferBondedEventFromCheckpoint({
    fromChainId,
    checkpoint
  })
  console.log(event)
...
`.trim()
  ...
  return (
    <Box>
      <Box mb={1}>
        <Typography variant="h5">Rails Gateway - Get Transfer Bonded Event From Checkpoint</Typography>
      </Box>
      <Box mb={4}>
...
export default RailsGatewayGetTransferBondedEventFromCheckpoint
