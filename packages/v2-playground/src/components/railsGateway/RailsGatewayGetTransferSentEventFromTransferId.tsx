...
export function RailsGatewayGetTransferSentEventFromTransferId (props: Props) {
  const cacheKey = 'railsGatewayGetTransferSentEventFromTransferId'
  const { sdk } = props
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const event = await sdk.getRailsGateway(fromChainId).helpers.getTransferSentEventFromTransferId(args)
...
async function main() {
  ...
  const event = await hop.getRailsGateway('${fromChainId}').helpers.getTransferSentEventFromTransferId({
    transferId
  })
  console.log(event)
}
...
`.trim()
  ...
  return (
    <Box>
      <Box mb={1}>
        <Typography variant="h5">Rails Gateway - Get Transfer Sent Event From Transfer ID</Typography>
      </Box>
      <Box mb={4}>
      ...
      <Box width="100%" display="flex" justifyContent="space-between" className={styles.container}>
        <Box mr={4} className={styles.formContainer}>
          <form onSubmit={handleSubmit}>
            ...
            <Box mb={2}>
              <Box mb={1}>
                <label>Transfer ID <small><em>(bytes32)</em></small> <small><em>This is the transferId from the <code>TransferSent</code> event</em></small></label>
              </Box>
