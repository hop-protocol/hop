...
export function StakingRegistryStakeHop (props: Props) {
  const cacheKey = 'stakingRegistryStakeHop'
  ...
  async function getSendTxData() {
    ...
    const txData = await stakingRegistry.populateTransaction.stakeHop(args)
    return txData
  }
...
async function main() {
  ...
  const txData = await hop.getRailsGateway('${fromChainId}').getStakingRegistry().populateTransaction.stakeHop({
    staker,
    amount
  })
  ${populateTxDataOnly ? (
...
`.trim()
  ...
  return (
    <Box>
      <Box mb={1}>
        <Typography variant="h5">Staking Registry - Stake HOP</Typography>
      </Box>
      <Box mb={4}>
        <Typography variant="subtitle1">Stake HOP for bonding</Typography>
      </Box>
        ...
        <Box mr={4} className={styles.formContainer}>
          <Box>
            <form onSubmit={handleSubmit}>
              <Box mb={2}>
                <Box mb={1}>
                  <label>Chain ID <small><em>(uint256)</em></small> <small><em>This is the chain to stake on</em></small></label>
                </Box>
              ...
              <Box mb={2}>
                <Box mb={1}>
                  <label>Amount <small><em>(uint256)</em></small> <small><em>Amount to stake</em></small></label>
                </Box>
              ...
              <Box mb={2} display="flex" justifyContent="center">
                ...
                {!!signer && (
                  <HighlightedButton loading={loading} fullWidth type="submit" variant="contained" size="large">{populateTxDataOnly ? 'Get tx data' : 'Stake'}</HighlightedButton>
                )}
              </Box>
            </form>
          </Box>
...
export default StakingRegistryStakeHop
