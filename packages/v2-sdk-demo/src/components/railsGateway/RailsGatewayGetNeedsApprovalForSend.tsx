...
export function RailsGatewayGetNeedsApprovalForSend (props: Props) {
  const cacheKey = 'railsGatewayGetNeedsApprovalForSend'
  const { sdk } = props
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const needsApproval = await sdk.getRailsGateway(fromChainId).helpers.getNeedsApprovalForSend(args)
...
`.trim()
  ...
  return (
    <Box>
      <Box mb={1}>
        <Typography variant="h5">Rails Gateway - Get Needs Approval For Send</Typography>
      </Box>
      <Box mb={4}>
...
export default RailsGatewayGetNeedsApprovalForSend
