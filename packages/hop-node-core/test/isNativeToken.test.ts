...
describe('isNativeToken', () => {
  it('isNativeToken', () => {
    ...
    let token = 'ETH'
    expect(isNativeToken(getChain(NetworkSlug.Mainnet, network).chainId, token)).toBe(true)
    ...
    network = ChainSlug.Polygon
    token = 'mAtIc'
    expect(isNativeToken(getChain(NetworkSlug.Mainnet, network).chainId, token)).toBe(false)
