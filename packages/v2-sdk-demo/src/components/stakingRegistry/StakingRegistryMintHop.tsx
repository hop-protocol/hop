...
export function StakingRegistryMinHop (props: Props) {
  const cacheKey = 'stakingRegistryMinHop'
  ...
  async function getSendTxData() {
    ...
    const txData = await stakingRegistry.populateTransaction.mint(args)
    return txData
  }
...
async function main() {
  ...
  const txData = await hop.getRailsGateway('${fromChainId}').getStakingRegistry().populateTransaction.mint({
    to,
    amount
  })
  ${populateTxDataOnly ? (
...
`.trim()
  ...
  return (
    <Box>
      <Box mb={1}>
        <Typography variant="h5">Staking Registry - Mint Testnet HOP</Typography>
      </Box>
      <Box mb={4}>
        <Typography variant="subtitle1">Mint HOP for testing</Typography>
      </Box>
        ...
        <Box mr={4} className={styles.formContainer}>
          <Box>
            <form onSubmit={handleSubmit}>
              <Box mb={2}>
                <Box mb={1}>
                  <label>Chain ID <small><em>(uint256)</em></small> <small><em>This is the chain to mint on</em></small></label>
                </Box>
              ...
              <Box mb={2}>
                ...
                <CustomTextField fullWidth placeholder="0x" value={recipient} onChange={(event: any) => setRecipient(event.target.value)} />
              </Box>

              <Box mb={2}>
                <Box mb={1}>
                  <label>Amount <small><em>(uint256)</em></small> <small><em>Amount to mint</em></small></label>
                </Box>
              ...
              <Box mb={2} display="flex" justifyContent="center">
                ...
                {!!signer && (
                  <HighlightedButton loading={loading} fullWidth type="submit" variant="contained" size="large">{populateTxDataOnly ? 'Get tx data' : 'Mint'}</HighlightedButton>
                )}
              </Box>
            </form>
          </Box>
...
export default StakingRegistryMinHop
