...
export function RailsGatewayUpdateClaimChain (props: Props) {
  const cacheKey = 'railsGatewayUpdateClaimChain'
  ...
  async function getSendTxData() {
    ...
    const txData = await sdk.getRailsGateway(fromChainId).populateTransaction.updateClaimChain(args)
    return txData
  }
...
async function main() {
  ...
  const txData = await hop.getRailsGateway('${fromChainId}').populateTransaction.updateClaimChain({
    pathId,
    transferDataHash,
    claimId
  })
...
`.trim()
  ...
  return (
    <Box>
      <Box mb={1}>
        <Typography variant="h5">Rails Gateway - Update Claim Chain</Typography>
      </Box>
      <Box mb={4}>
        <Typography variant="subtitle1">Update claim chain</Typography>
      </Box>
        ...
        <Box mr={4} className={styles.formContainer}>
          <Box>
            <form onSubmit={handleSubmit}>
              ...
              <Box mb={2}>
                <Box mb={1}>
                  <label>Claim ID <small><em>(bytes32)</em></small> <small><em>Claim ID to use</em></small></label>
                </Box>
              ...
              <Box mb={2} display="flex" justifyContent="center">
                ...
                {!!signer && (
                  <HighlightedButton loading={loading} fullWidth type="submit" variant="contained" size="large">{populateTxDataOnly ? 'Get tx data' : 'Send Update Tx'}</HighlightedButton>
                )}
              </Box>
            </form>
          </Box>
...
export default RailsGatewayUpdateClaimChain
