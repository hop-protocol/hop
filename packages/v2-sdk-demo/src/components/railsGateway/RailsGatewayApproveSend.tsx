...
export function RailsGatewayApproveSend (props: Props) {
  const cacheKey = 'railsGatewayApproveSend'
  ...
  async function getSendTxData() {
    ...
    const txData = await sdk.getRailsGateway(fromChainId).populateTransaction.approveSend(args)
    return txData
  }
...
async function main() {
  ...
  const txData = await hop.getRailsGateway('${fromChainId}').populateTransaction.approveSend({
    pathId,
    amount
  })
  ${populateTxDataOnly ? (
...
`.trim()
  ...
  return (
    <Box>
      <Box mb={1}>
        <Typography variant="h5">Rails Gateway - Approve Send</Typography>
      </Box>
      <Box mb={4}>
