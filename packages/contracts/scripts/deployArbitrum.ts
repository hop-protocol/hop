...
async function deployArbitrum () {
  ...
  let kovanProvider = new ethers.providers.JsonRpcProvider('https://kovan.rpc.authereum.com')
  // let arbitrumProvider = new ethers.providers.JsonRpcProvider('https://kovan2.arbitrum.io/rpc')

  // Instantiate the wallets
  user = ethers.Wallet.fromMnemonic(process.env.USER_MNEMONIC)
  user.connect(kovanProvider)

  liquidityProvider = ethers.Wallet.fromMnemonic(process.env.LIQUIDITY_PROVIDER_MNEMONIC)
  liquidityProvider.connect(localProvider)
