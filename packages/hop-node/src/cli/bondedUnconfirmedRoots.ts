...
export async function main (source: any) {
  ...
  const rootsCommitted = await getRootsCommitted(chain, token)
  ...
  for (const rootHash in rootsCommitted) {

    let isConfirmed = false
    for (const rootConfirmed of rootsConfirmed) {
      if (rootHash === rootConfirmed.rootHash) {
        isConfirmed = true
      }
    }
    ...
    const rootData = rootsCommitted[rootHash]
    bondedUnconfirmedRootAmount = bondedUnconfirmedRootAmount.add(rootData.totalAmount)
  }

  return bondedUnconfirmedRootAmount
}

async function getRootsCommitted (chain: string, token: string): Promise<Record<string, RootsCommitted>> {
  ...
  for (const nonRpcRootHash in nonRpcRoots) {
    if (rootsCommitted[nonRpcRootHash]) {
      throw new Error(`Duplicate root hash: ${nonRpcRootHash}`)
    }

    const nonRpcRootData = nonRpcRoots[nonRpcRootHash]
    if (nonRpcRootData.token !== token) continue
    if (chainIdToSlug(nonRpcRootData.sourceChainId) !== chain) continue

    rootsCommitted[nonRpcRootHash] = {
      rootHash: nonRpcRootData.rootHash,
      sourceChainId: chainSlugToId(chain).toString(),
      totalAmount: BigNumber.from(nonRpcRootData.totalAmount),
      destinationChainId: nonRpcRootData.destinationChainId,
      rootCommittedAt: nonRpcRootData.rootCommittedAt,
      transactionHash: nonRpcRootData.transactionHash,
      token: nonRpcRootData.token
    }
  }
