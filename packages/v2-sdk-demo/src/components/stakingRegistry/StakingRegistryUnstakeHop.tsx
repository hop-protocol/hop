...
export function StakingRegistryUnstakeHop (props: Props) {
  const cacheKey = 'stakingRegistryUnstakeHop'
...
async function main() {
  ...
  const txData = await hop.getRailsGateway('${fromChainId}').getStakingRegistry().populateTransaction.unstakeHop({
    amount
  })
  ${populateTxDataOnly ? (
  'console.log(txData)'
...
`.trim()
  ...
  return (
    <Box>
      <Box mb={1}>
        <Typography variant="h5">Staking Registry - Unstake HOP</Typography>
      </Box>
      <Box mb={4}>
        <Typography variant="subtitle1">Unstake HOP from staking registry</Typography>
      </Box>
        ...
        <Box mr={4} className={styles.formContainer}>
          <Box>
            <form onSubmit={handleSubmit}>
              <Box mb={2}>
                <Box mb={1}>
                  <label>Chain ID <small><em>(uint256)</em></small> <small><em>This is the chain to unstake from</em></small></label>
                </Box>
              ...
              <Box mb={2}>
                <Box mb={1}>
                  <label>Amount <small><em>(uint256)</em></small> <small><em>Amount to unstake</em></small></label>
                </Box>
              ...
              <Box mb={2} display="flex" justifyContent="center">
                ...
                {!!signer && (
                  <HighlightedButton loading={loading} fullWidth type="submit" variant="contained" size="large">{populateTxDataOnly ? 'Get tx data' : 'Unstake'}</HighlightedButton>
                )}
              </Box>
            </form>
          </Box>
...
export default StakingRegistryUnstakeHop
