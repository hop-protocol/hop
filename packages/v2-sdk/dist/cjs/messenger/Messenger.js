...
const MerkleTree_js_1 = require("#utils/MerkleTree.js");
...
const MockExecutor__factory_js_1 = require("#contracts/factories/MockExecutor__factory.js");
const FeesSentToHub_js_1 = require("#messenger/events/FeesSentToHub.js");
const index_js_2 = require("#gasPriceOracle/index.js");
const index_js_3 = require("#error/index.js");
const { formatEther, formatUnits, parseEther } = ethers_1.utils;
class Messenger extends index_js_1.Base {
    constructor(config) {
        super({ network: config.network, signer: config.signer, contractAddresses: config.contractAddresses });
        this.batchBlocks = 1000;
        this.gasPriceOracle = new index_js_2.GasPriceOracle(this.network);
    }
    connect(signer) {
    ...
    getSpokeMessageBridgeContractAddress(chainId) {
        if (!this.utils.isValidChainId(chainId)) {
            throw new index_js_3.InputError(`Invalid chainId "${chainId}"`);
        }
    ...
    async getBundleCommittedEvents(input) {
        ...
        if (!chainId) {
            throw new index_js_3.InputError('chainId is required');
        }
        if (!fromBlock) {
            throw new index_js_3.InputError('fromBlock is required');
        }
        if (!this.utils.isValidChainId(chainId)) {
            throw new index_js_3.InputError(`Invalid chainId "${chainId}"`);
        }
        if (!this.utils.isValidFilterBlock(fromBlock)) {
            throw new index_js_3.InputError(`Invalid fromBlock "${fromBlock}"`);
        }
        if (toBlock && this.utils.isValidFilterBlock(toBlock)) {
            throw new index_js_3.InputError(`Invalid toBlock "${toBlock}"`);
        }
        const provider = this.getRpcProviderForChainId(chainId);
        if (!provider) {
            throw new index_js_3.ConfigError(`Provider not found for chainId: ${chainId}`);
        }
        const address = this.getSpokeMessageBridgeContractAddress(chainId);
        if (!address) {
            throw new index_js_3.ConfigError(`Contract address not found for chainId: ${chainId}`);
        }
    ...
    async getHasAuctionStarted(input) {
        ...
        if (!this.utils.isValidChainId(fromChainId)) {
            throw new index_js_3.InputError(`Invalid fromChainId "${fromChainId}"`);
        }
        if (!bundleCommittedEvent) {
            throw new index_js_3.InputError('bundleCommittedEvent is required');
        }
        const { commitTime, toChainId } = bundleCommittedEvent;
        const exitTime = await this.getSpokeExitTime({ fromChainId, toChainId });
        return commitTime + exitTime < luxon_1.DateTime.utc().toSeconds();
    }
    async getSpokeExitTime(input) {
        ...
        if (!this.utils.isValidChainId(toChainId)) {
            throw new index_js_3.InputError(`Invalid toChainId "${toChainId}"`);
        }
        const provider = this.getRpcProviderForChainId(toChainId);
        if (!provider) {
            throw new index_js_3.InputError(`Invalid chainId "${toChainId}", provider not found`);
        }
        const address = this.getHubMessageBridgeContractAddress(toChainId);
        if (!address) {
            throw new index_js_3.InputError(`Invalid chain: ${toChainId}`);
        }
        const hubMessageBridge = HubMessageBridge__factory_js_1.HubMessageBridge__factory.connect(address, provider);
    ...
    async getRelayReward(input) {
        ...
        if (!provider) {
            throw new index_js_3.InputError(`Invalid chainId "${fromChainId}", provider not found`);
        }
    ...
    async getEstimatedTxCostForForwardMessage(input) {
        ...
        const provider = this.getRpcProviderForChainId(chainId);
        if (!provider) {
            throw new index_js_3.InputError(`Invalid chainId "${chainId}", provider not found`);
        }
        const estimatedGas = ethers_1.BigNumber.from(1000000); // TODO
    ...
    async exitBundle(input) {
        ...
        if (bundleCommittedTransactionHash) {
            if (!this.utils.isValidTxHash(bundleCommittedTransactionHash)) {
                throw new index_js_3.InputError(`Invalid transaction hash "${bundleCommittedTransactionHash}"`);
            }
        }
        else if (bundleCommittedEvent) {
            const { eventLog, context } = bundleCommittedEvent;
            if (!eventLog) {
                throw new index_js_3.InputError('eventLog is required');
            }
            bundleCommittedTransactionHash = eventLog.transactionHash ?? context?.transactionHash;
        }
        else {
            throw new index_js_3.InputError('bundleCommittedEvent or bundleCommittedTransactionHash is required');
        }
        if (!bundleCommittedTransactionHash) {
            throw new index_js_3.InputError('expected bundle comitted transaction hash');
        }
        ...
        if (!exitRelayer) {
            throw new index_js_3.ConfigError(`Exit relayer not found for chainId "${fromChainId}"`);
        }
    ...
    async getIsL2TxHashExited(input) {
        ...
        if (!this.utils.isValidTxHash(transactionHash)) {
            throw new index_js_3.InputError(`Invalid transaction hash "${transactionHash}"`);
        }
        const l1Provider = this.getRpcProviderForChainId(this.l1ChainId);
        const l2Provider = this.getRpcProviderForChainId(fromChainId);
        let exitRelayer;
        ...
        return {
            sendMessage: async (input) => {
                ...
                if (fromChainId?.toString() === toChainId?.toString()) {
                    throw new index_js_3.InputError('fromChainId and toChainId must be different');
                }
                if (!toAddress) {
                    throw new index_js_3.InputError('toAddress is required');
                }
                if (!this.utils.isValidAddress(toAddress)) {
                    throw new index_js_3.InputError(`Invalid toAddress "${toAddress}"`);
                }
                if (!toCalldata) {
                    toCalldata = '0x';
                }
                if (!this.utils.isValidBytes(toCalldata)) {
                    throw new index_js_3.InputError(`Invalid toCalldata "${toCalldata}"`);
                }
                const provider = this.getRpcProviderForChainId(fromChainId);
                if (!provider) {
                    throw new index_js_3.InputError(`Invalid chainId, "${fromChainId}", provider not found`);
                ...
                if (!address) {
                    throw new index_js_3.ConfigError(`Invalid address, not found for chainId "${fromChainId}"`);
                }
                const spokeMessageBridge = SpokeMessageBridge__factory_js_1.SpokeMessageBridge__factory.connect(address, provider);
            ...
            relayMessage: async (input) => {
                ...
                if (!bundleProof) {
                    throw new index_js_3.InputError('bundleProof is required');
                }
                if (!this.isValidBundleProof(bundleProof)) {
                    throw new index_js_3.InputError('Invalid bundleProof');
                }
                const provider = this.getRpcProviderForChainId(toChainId);
                if (!provider) {
                    throw new index_js_3.InputError(`Invalid chainId "${toChainId}", provider not found`);
                ...
                const address = this.getHubMessageBridgeContractAddress(toChainId);
                if (!address) {
                    throw new index_js_3.InputError(`Invalid chainId "${toChainId}", address not found`);
                }
                const hubMessageBridge = HubMessageBridge__factory_js_1.HubMessageBridge__factory.connect(address, provider);
            ...
            execute: async (input) => {
                ...
                if (!this.utils.isValidBytes32(messageId)) {
                    throw new index_js_3.InputError(`Invalid messageId "${messageId}"`);
                }
                if (!this.utils.isValidAddress(fromAddress)) {
                    throw new index_js_3.InputError(`Invalid fromAddress "${fromAddress}"`);
                }
                if (!this.utils.isValidAddress(toAddress)) {
                    throw new index_js_3.InputError(`Invalid toAddress "${toAddress}"`);
                }
                if (!this.utils.isValidBytes(toCalldata)) {
                    throw new index_js_3.InputError(`Invalid calldata "${toCalldata}"`);
                }
                const address = this.getExecutorContractAddress(toChainId);
                if (!address) {
                    throw new index_js_3.InputError(`Invalid address, not found for chainId "${toChainId}"`);
                }
                const provider = this.getRpcProviderForChainId(toChainId);
                const mockExecutor = MockExecutor__factory_js_1.MockExecutor__factory.connect(address, provider);
    ...
    async getIsBundleSet(input) {
        ...
        const provider = this.getRpcProviderForChainId(toChainId);
        if (!provider) {
            throw new index_js_3.ConfigError(`Provider not found for chainId "${toChainId}"`);
        }
        const address = this.getSpokeMessageBridgeContractAddress(toChainId);
        if (!address) {
            throw new index_js_3.ConfigError(`Contract address not found for chainId "${toChainId}"`);
        }
        const hubMessageBridge = HubMessageBridge__factory_js_1.HubMessageBridge__factory.connect(address, provider);
        ...
        return ethers_1.BigNumber.from(entity.root).gt(0) && Number(entity.fromChainId.toString()) === fromChainId;
    }
    async getMessageSentEventFromTransactionReceipt(input) {
        ...
        if (!receipt) {
            throw new index_js_3.InputError('receipt is required');
        }
        const provider = this.getRpcProviderForChainId(fromChainId);
        if (!provider) {
            throw new index_js_3.ConfigError(`Provider not found for chainId "${fromChainId}"`);
        }
        const address = this.getSpokeMessageBridgeContractAddress(fromChainId);
        if (!address) {
            throw new index_js_3.ConfigError(`Contract address not found for chainId: ${fromChainId}`);
        }
        const eventFetcher = new MessageSent_js_1.MessageSentEventFetcher(provider, fromChainId, this.batchBlocks, address);
    ...
    async getMessageSentEventFromTransactionHash(input) {
        ...
        if (!transactionHash) {
            throw new index_js_3.InputError('transactionHash is required');
        }
        if (!this.utils.isValidTxHash(transactionHash)) {
            throw new index_js_3.InputError(`Invalid transaction hash "${transactionHash}"`);
        }
    ...
    async getMessageBundledEventFromMessageId(input) {
        ...
        const eventFetcher = new MessageBundled_js_1.MessageBundledEventFetcher(provider, fromChainId, 0, address);
    ...
    async getMessageSentEventFromMessageId(input) {
        const { fromChainId, messageId } = input;
        if (!fromChainId) {
            throw new index_js_3.InputError('fromChainId is required');
        }
        if (!messageId) {
            throw new index_js_3.InputError('messageId is required');
        }
        if (!this.utils.isValidChainId(fromChainId)) {
            throw new index_js_3.InputError(`Invalid fromChainId ${fromChainId}""`);
        }
        ...
        const eventFetcher = new MessageSent_js_1.MessageSentEventFetcher(provider, fromChainId, 0, address);
        const filter = eventFetcher.getMessageIdFilter(messageId);
        const toBlock = await provider.getBlockNumber();
        const fromBlock = 0; // endBlock - 100_000
        const events = await eventFetcher.getEventsWithFilter(filter, fromBlock, toBlock);
    ...
    async getMessageExecutedEventFromMessageId(input) {
        ...
        const eventFetcher = new MessageExecuted_js_1.MessageExecutedEventFetcher(provider, toChainId, 0, address);
        const filter = eventFetcher.getMessageIdFilter(messageId);
        const toBlock = await provider.getBlockNumber();
        const fromBlock = 0; // endBlock - 100_000
        const events = await eventFetcher.getEventsWithFilter(filter, fromBlock, toBlock);
    ...
    async getMessageBundledEventFromTransactionHash(input) {
        ...
        const eventFetcher = new MessageBundled_js_1.MessageBundledEventFetcher(provider, fromChainId, this.batchBlocks, address);
        const events = eventFetcher.decodeEventsFromTransactionReceipt(receipt);
        return events?.[0] ?? null;
    }
    ...
    async getMessageBundleIdFromTransactionHash(input) {
        ...
        if (!this.utils.isValidTxHash(transactionHash)) {
            throw new index_js_3.InputError(`Invalid transactionHash "${transactionHash}"`);
        }
    ...
    async getMessageBundledEventsForBundleId(input) {
        ...
        if (!this.utils.isValidBytes32(bundleId)) {
            throw new index_js_3.InputError(`Invalid bundleId "${bundleId}"`);
        }
        const provider = this.getRpcProviderForChainId(fromChainId);
        if (!provider) {
            throw new index_js_3.ConfigError(`Provider not found for chainId: ${fromChainId}`);
    ...
    async getMerkleProofForMessageId(input) {
        ...
        if (!targetMessageId) {
            throw new index_js_3.InputError('targetMessageId is required');
        }
        if (!Array.isArray(messageIds)) {
            throw new index_js_3.InputError('messageIds is required and must be an array');
        }
        if (!messageIds.every((item) => this.utils.isValidBytes32(item))) {
            throw new index_js_3.InputError('Invalid messageIds');
        }
        if (!this.utils.isValidBytes32(targetMessageId)) {
            throw new index_js_3.InputError(`Invalid targetMessageId "${targetMessageId}"`);
        }
        const tree = MerkleTree_js_1.MerkleTree.from(messageIds);
