...
export const CumulativeVolumeChart: React.FC<CumulativeVolumeChartProps> = ({ 
    ...
    const loadChartJs = async () => {
      ...
      try {
        ...
        datasets.forEach((dataset, index) => {
          ...
          console.log(`Creating cumulative chart for ${tokenSymbol} with min=${yMin}, max=${yMax}, data:`, numericData)
  ...
  }, [cumulativeVolumeStats, tokenData, useLogScale])
  ...
  const hasData = cumulativeVolumeStats?.data?.datasets?.some(dataset => 
  ...
  return (
    <Box sx={{ width: '100%', p: 2 }}>
      ...
      {!loading && !error && (!hasData || !cumulativeVolumeStats?.data || cumulativeVolumeStats.data.datasets.length === 0) && (
        <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: 300 }}>
          <Typography>
            No cumulative volume data available for the selected period
          </Typography>
        </Box>
      )}
      
      {!loading && !error && hasData && cumulativeVolumeStats?.data && (
      ...
      <Box mt={1}>
        <Typography variant="caption" color="textSecondary">
          Last updated: {cumulativeVolumeStats?.lastUpdated 
            ? new Date(cumulativeVolumeStats.lastUpdated).toLocaleString() 
            : 'N/A'}
        </Typography>
      </Box>
    </Box>
