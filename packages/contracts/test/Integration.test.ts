...
describe("Full story", () => {
  ...
  beforeEach(async () => {
    ...
    l1_wormhole = await L1_Wormhole.deploy(l1_messenger.address, l1_poolToken.address)
    ...
    await l1_wormhole.setL2Bridge(l2_wormhole.address)
    await l2_wormhole.setL1Bridge(l1_wormhole.address)
  ...
  it('Should complete the full story', async () => {
    ...
    await l1_wormhole.connect(liquidityProvider).sendToL2(await liquidityProvider.getAddress(), LIQUIDITY_PROVIDER_INITIAL_BALANCE.div(2))
    await l2_messenger.relayNextMessage()
    ...
    await l2_wormhole.connect(liquidityProvider).approve(l2_uniswapRouter.address, LIQUIDITY_PROVIDER_INITIAL_BALANCE.div(2))
    ...
    await expectBalanceOf(l2_wormhole, liquidityProvider, '0')

    const uniswapPairAddress: string = await l2_uniswapFactory.getPair(l2_ovmBridge.address, l2_wormhole.address)
    ...
    await expectBalanceOf(l2_wormhole, uniswapPair, LIQUIDITY_PROVIDER_INITIAL_BALANCE.div(2))
    ...
    await expectBalanceOf(l2_wormhole, user, '99')
    ...
    await l2_wormhole.connect(user).sendToMainnet(transfer.sender, transfer.amount, transfer.nonce)
    await l2_wormhole.commitTransfers()
