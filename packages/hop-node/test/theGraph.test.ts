...
describe.skip('getTransferRoots', () => {
  it('gnosis', async () => {
    const transferRoots = await getTransferRoots(ChainSlug.Gnosis, 'USDC')
...
describe.skip('getTransferRoot', () => {
  it('gnosis', async () => {
    ...
    const transferRoot = await getTransferRoot(ChainSlug.Gnosis, 'USDC', transferRootHash)
...
describe.skip('getTransfer', () => {
  it('gnosis - 1', async () => {
    ...
    const transfer = await getTransfer(ChainSlug.Gnosis, 'USDC', transferId)
...
describe.skip('getBondedWithdrawal', () => {
  it('polygon', async () => {
    const transferId = '0xb7329b58f3ab879e40df7d2fabf21e591a35adb42803cc4b676fa726a6252ab7'
    const item = await getBondedWithdrawal(ChainSlug.Polygon, 'USDC', transferId)
...
describe.skip('getBondedWithdrawals', () => {
  it('polygon', async () => {
    const items = await getBondedWithdrawals(ChainSlug.Ethereum, 'USDC')
...
describe.skip('check bonded withdrawals without a transfer', () => {
  it('polygon', async () => {
    ...
    for (const item of items.slice(i)) {
      try {
        let transfer = await getTransfer(ChainSlug.Gnosis, 'USDC', item.transferId)
        if (!transfer) {
          transfer = await getTransfer(ChainSlug.Polygon, 'USDC', item.transferId)
        }
        if (!transfer) {
...
describe.skip('getSubgraphLastBlockSynced', () => {
  it('gnosis', async () => {
    const block = await getSubgraphLastBlockSynced(ChainSlug.Gnosis)
    console.log(block)
...
describe.skip('getUnbondedTransfers', () => {
  it('fetchTransfers', async () => {
    ...
    const transfers = await fetchTransfers(ChainSlug.Polygon, startTime, endTime)
