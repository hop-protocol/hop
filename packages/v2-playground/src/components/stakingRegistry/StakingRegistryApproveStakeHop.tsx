...
export function StakingRegistryApproveStakeHop (props: Props) {
  const cacheKey = 'stakingRegistryApproveStakeHop'
  ...
  async function getSendTxData() {
    ...
    const txData = await sdk.getRailsGateway(fromChainId).getStakingRegistry().populateTransaction.approveStake(args)
    return txData
  }
...
async function main() {
  ...
  const txData = await hop.getRailsGateway('${fromChainId}').getStakingRegistry().populateTransaction.approveStake({
    amount
  })
  ${populateTxDataOnly ? (
  'console.log(txData)'
...
`.trim()
  ...
  return (
    <Box>
      <Box mb={1}>
        <Typography variant="h5">Staking Registry - Approve Stake HOP</Typography>
      </Box>
      <Box mb={4}>
        <Typography variant="subtitle1">Approve tokens for staking HOP</Typography>
      </Box>
        ...
        <Box mr={4} className={styles.formContainer}>
          <Box>
            <form onSubmit={handleSubmit}>
              <Box mb={2}>
                <Box mb={1}>
                  <label>Chain ID <small><em>(uint256)</em></small> <small><em>This is the chain to approve on</em></small></label>
                </Box>
...
export default StakingRegistryApproveStakeHop
