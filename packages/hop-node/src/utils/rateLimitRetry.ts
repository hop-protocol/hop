  ...
  return async (...args: Parameters<FN>): Promise<Awaited<ReturnType<FN>>> => {
    ...
    while (true) {
      try {
        ...
        const blockHashValidatorContractErrorRegex = /BHV:/
        ...
        const isBlockHashValidatorContractError = blockHashValidatorContractErrorRegex.test(errMsg)
        ...
        const shouldNotRetryErrors = (isOversizedDataError || isBridgeContractError || isNonceTooLowErrorError || isEstimateGasFailedError || isAlreadyKnownError || isFeeTooLowError || isCallLookupRevertError || isBlockHashValidatorContractError)
