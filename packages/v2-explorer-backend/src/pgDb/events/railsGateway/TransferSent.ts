...
export class TransferSentTable extends EventDb {
  ...
  async getDetailedDailyVolumeData(opts: DailyVolumeStatsInput = {}): Promise<any[]> {
    // Get current timestamp in seconds
    const now = Math.floor(Date.now() / 1000)
    ...
    console.log(`Analyzing detailed volume data from ${new Date(startTimestamp * 1000).toISOString()} to ${new Date(safeEndTimestamp * 1000).toISOString()}`)

    const pathIdFilter = pathId ? `AND e.path_id = '${pathId}'` : ''
    const args = [startTimestamp, safeEndTimestamp]
    
    try {
      // Direct approach to get daily summaries from deduplicated transfers
      const query = `
        -- Get summary data by date and token with deduplication by transfer_id
        WITH unique_transfers AS (
          SELECT DISTINCT ON (e.transfer_id)
            e.transfer_id,
            e.amount,
        ...
        FROM 
          unique_transfers
        GROUP BY 
          date, token_symbol, token_decimals
        ORDER BY 
          date DESC, token_symbol ASC
      `

      const results = await this.db.any(query, args)
      console.log(`Found ${results.length} daily transfer summaries`)
      
      // Format the results with appropriate decimal handling
      const summaryResults = results.map((item: any) => {
        // Format the amount with proper decimals
        const decimals = parseInt(item.token_decimals)
        let formattedAmount = item.total_raw_amount
        
        if (decimals > 0) {
          try {
            // Format raw value to display value based on decimals
            if (formattedAmount === '0') {
              formattedAmount = '0'
            } else {
              const valueStr = formattedAmount.toString()
        ...
        return {
          ...
          transferCount: parseInt(item.transfer_count),
          uniqueTransactions: parseInt(item.unique_transactions),
          totalRawAmount: item.total_raw_amount,
          formattedAmount
        }
      })
      
      // Sort by date and token
      summaryResults.sort((a: any, b: any) => {
        if (a.date === b.date) {
