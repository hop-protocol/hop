...
export type Props = {
  ...
  error: string
  getSignerOrRequestWallet: () => Promise<providers.JsonRpcSigner>
  checkConnectedNetworkIdOrThrow: (chainId: number) => Promise<void>
}
...
const Web3ContextProvider: FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const update = async () => {
      try {
        ...
        console.error('web3 react activate error:', err)
        setWeb3ModalChoice('')
        setWalletChoiceLoading(false)
      }
    }
  ...
  const checkConnectedNetworkId = async (wantChainId?: number): Promise<boolean> => {
    ...
    console.debug(`checkConnectedNetworkId want: ${wantChainId}, have: ${haveChainId}`)
