...
type CumulativeVolumeStatsProps = {
  days?: number
  pathId?: string
...
const fetchCumulativeVolumeStats = async (options: CumulativeVolumeStatsProps = {}): Promise<CumulativeVolumeStats> => {
  ...
  let url = `${protocol}://${hostname}/api/?pathname=/stats/cumulative-volume`
  ...
  console.log('Cumulative volume API response:', data);
...
export const useFetchCumulativeVolumeStats = (options: CumulativeVolumeStatsProps = {}): { 
  cumulativeVolumeStats: CumulativeVolumeStats | null, 
  loading: boolean, 
  error: string | null
} => {
  const queryKey = [
    'cumulativeVolumeStats',
    options.days,
    options.pathId,
  ...
  const { data, isLoading, error } = useQuery<CumulativeVolumeStats, Error>(
    queryKey,
    () => fetchCumulativeVolumeStats(options),
    {
  ...
  return {
    cumulativeVolumeStats: data || null,
    loading: isLoading,
