...
exports.getUSDCSwapParams = void 0;
...
async function getUSDCSwapParams(options) {
    const { network, chainId, provider, amountIn, recipient, getQuote = false } = options;
    const chain = (0, chainIdToSlug_js_1.chainIdToSlug)(network, chainId);
    ...
    const fromTokenContract = new ethers_1.Contract(tokens[0], abi_1.erc20Abi, provider);
    const toTokenContract = new ethers_1.Contract(tokens[1], abi_1.erc20Abi, provider);
    const fromTokenDecimals = await fromTokenContract.decimals();
    const toTokenDecimals = await toTokenContract.decimals();
    // Define the tokens
    const tokenA = new sdk_core_1.Token(chainId, tokens[0], fromTokenDecimals, fromToken);
    const tokenB = new sdk_core_1.Token(chainId, tokens[1], toTokenDecimals, toToken);
    // Create a pool
    const poolContract = new ethers_1.Contract(poolAddress, abi_2.UniswapV3Pool, provider);
    // Fetch pool details
    ...
    const pool = new v3_sdk_1.Pool(tokenA, tokenB, feeTier, sqrtPriceX96, liquidity, tick, [
        {
            index: (0, v3_sdk_1.nearestUsableTick)(v3_sdk_1.TickMath.MIN_TICK, v3_sdk_1.TICK_SPACINGS[feeTier]),
            liquidityNet: liquidity,
            liquidityGross: liquidity
        },
        {
            index: (0, v3_sdk_1.nearestUsableTick)(v3_sdk_1.TickMath.MAX_TICK, v3_sdk_1.TICK_SPACINGS[feeTier]),
            liquidityNet: ethers_1.BigNumber.from(liquidity).mul(-1).toString(),
            liquidityGross: liquidity
        }
    ]);
    // Define a route
    const route = new v3_sdk_1.Route([pool], tokenA, tokenB);
    // Create a trade for a given input amount
    const amountInTokenA = sdk_core_1.CurrencyAmount.fromRawAmount(tokenA, amountIn.toString());
    const trade = await v3_sdk_1.Trade.exactIn(route, amountInTokenA);
    // Calculate slippage tolerance and set deadline
    const slippageTolerance = new sdk_core_1.Percent('50', '10000'); // 0.5%
    const deadline = Math.floor(Date.now() / 1000) + 60 * 20; // 20 minutes from now
    const exactOutput = false;
    const swapParams = {
        path: (0, v3_sdk_1.encodeRouteToPath)(route, exactOutput),
        recipient: recipient,
        // deadline: deadline.toString(),
        amountIn: (0, utils_js_1.parseUnits)(trade.inputAmount.toExact(), fromTokenDecimals).toString(),
        amountOutMinimum: (0, utils_js_1.parseUnits)(trade.minimumAmountOut(slippageTolerance).toExact(), toTokenDecimals).toString(),
    };
    let quotedAmountOut;
    let quotedAmountOutFormatted;
    if (getQuote) {
        if (ethers_1.BigNumber.from(amountIn).eq(0)) {
            quotedAmountOut = ethers_1.BigNumber.from(0);
            quotedAmountOutFormatted = '0';
        }
        else {
            const quoterContract = new ethers_1.Contract(quoter, abi_1.UniswapQuoterV2Abi, provider);
            ...
            //   0
            // )
            quotedAmountOut = await quoterContract.callStatic.quoteExactInput((0, v3_sdk_1.encodeRouteToPath)(route, exactOutput), amountIn.toString());
            quotedAmountOutFormatted = (0, utils_js_1.formatUnits)(quotedAmountOut, toTokenDecimals);
        }
    }
    return {
        swapParams,
...
exports.getUSDCSwapParams = getUSDCSwapParams;
