...
export function RailsGatewayGetTransferBondedEventFromTxHash (props: Props) {
  const cacheKey = 'railsGatewayGetTransferBondedEventFromTxHash'
  const { sdk } = props
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const event = await sdk.getRailsGateway(fromChainId).helpers.getTransferBondedEventFromTransactionHash(args)
...
async function main() {
  ...
  const event = await hop.getRailsGateway('${fromChainId}').helpers.getTransferBondedEventFromTransactionHash({
    transactionHash
  })
  console.log(event)
}
...
`.trim()
  ...
  return (
    <Box>
      <Box mb={1}>
        <Typography variant="h5">Rails Gateway - Get Transfer Bonded Event From Transaction Hash</Typography>
      </Box>
      <Box mb={4}>
      ...
      <Box width="100%" display="flex" justifyContent="space-between" className={styles.container}>
        <Box mr={4} className={styles.formContainer}>
          <form onSubmit={handleSubmit}>
            ...
            <Box mb={2}>
              <Box mb={1}>
                <label>Transaction Hash <small><em>(hex string)</em></small> <small><em>This is the transaction hash of the origin transfer bonded transaction</em></small></label>
              </Box>
...
export default RailsGatewayGetTransferBondedEventFromTxHash
