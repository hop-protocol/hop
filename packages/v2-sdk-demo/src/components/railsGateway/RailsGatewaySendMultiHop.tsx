...
export function RailsGatewaySendMultiHop (props: Props) {
  const cacheKey = 'railsGatewaySendMultiHop'
  ...
  async function getSendTxData() {
    ...
    const txData = await sdk.railsGateway.populateTransaction.sendMultiHop(args)
    return txData
  }
...
async function main() {
  ...
  const txData = await hop.railsGateway.populateTransaction.sendMultiHop({
    to,
    amount,
    hops
  })
...
`.trim()
  ...
  return (
    <Box>
      <Box mb={1}>
        <Typography variant="h5">Rails Gateway - Send Multi Hop</Typography>
      </Box>
      <Box mb={4}>
        <Typography variant="subtitle1">Send tokens to a destination chain via multiple hops</Typography>
      </Box>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <Stepper orientation="vertical">
                {hops.map((hop: any, index: number) => {
                  ...
                  function setPathId (value: string) {
                    const newHops = [...hops]
...
export default RailsGatewaySendMultiHop
