...
export const parseFrames = async (data: Buffer | string): Promise<Frame[]> => {
  ...
  if (buf.length > 0) {
    try {
      const { frame, offset } = await unmarshalBinary(buf)
      frames.push(frame)
...
const unmarshalBinary = async (buf: Buffer): Promise<UnmarshalBinaryRes> => {
  let offset: number = 0

  if (buf.length < offset + DataLengths.ChannelIDLength) {
    throw new Error('reading channel_id: unexpected EOF')
  }
  const channelId: string = buf.subarray(offset, offset + DataLengths.ChannelIDLength).toString('hex')
  offset += DataLengths.ChannelIDLength

  if (buf.length < offset + DataLengths.FrameNumberLength) {
    throw new Error('reading frameNumber: unexpected EOF')
  }
  const frameNumber: number = buf.readUInt16BE(offset)
  offset += DataLengths.FrameNumberLength

  if (buf.length < offset + DataLengths.FrameDataLength) {
    throw new Error('reading frameDataLength: unexpected EOF')
  }
  const frameLength: number = buf.readUInt32BE(offset)
  offset += DataLengths.FrameDataLength
