...
async function updateChart (data: any[]) {
  ...
  const render = () => {
    window.d3.select('#chart svg').remove()
    const chart = window.d3.select('#chart').append('svg').chart('Sankey.Path')
    chart
      .name(label)
                ...
                return (
                  <tr key={index}>
                    ...
                    <td className="transferTx">
                      <a className="clipboard" data-clipboard-text={x.transactionHash} rel="noreferrer noopener" title="Copy transaction hash to clipboard" onClick={() => { event.target.innerText='✅';setTimeout(()=>event.target.innerText='📋',1000)}}>📋</a>
