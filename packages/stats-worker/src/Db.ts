...
class Db {
  ...
  constructor () {
    this.db.serialize(() => {
      ...
      this.migrations[13] = () => {
        this.db.run(
          'ALTER TABLE bonder_balances ADD COLUMN restaked_eth_amount NUMERIC;'
        )
      }
      this.migrations[14] = () => {
        this.db.run(
  ...
  async upsertBonderBalances (
    ...
    await this.tilReady()
    const stmt = this.db.prepare(`
      INSERT OR REPLACE INTO bonder_balances (
        id,
        token,
    ...
    stmt.run({
      $id: uuid(),
      $token: token,
      $polygonBlockNumber: polygonBlockNumber,
      $polygonCanonicalAmount: polygonCanonicalAmount,
      $polygonHTokenAmount: polygonHTokenAmount,
      $polygonNativeAmount: polygonNativeAmount,
      $gnosisBlockNumber: gnosisBlockNumber,
      $gnosisCanonicalAmount: gnosisCanonicalAmount,
      $gnosisHTokenAmount: gnosisHTokenAmount,
      $gnosisNativeAmount: gnosisNativeAmount,
      $arbitrumBlockNumber: arbitrumBlockNumber,
      $arbitrumCanonicalAmount: arbitrumCanonicalAmount,
      $arbitrumHTokenAmount: arbitrumHTokenAmount,
      $arbitrumNativeAmount: arbitrumNativeAmount,
      $arbitrumAliasAmount: arbitrumAliasAmount,
      $optimismBlockNumber: optimismBlockNumber,
      $optimismCanonicalAmount: optimismCanonicalAmount,
      $optimismHTokenAmount: optimismHTokenAmount,
      $optimismNativeAmount: optimismNativeAmount,
      $ethereumBlockNumber: ethereumBlockNumber,
      $ethereumCanonicalAmount: ethereumCanonicalAmount,
      $ethereumNativeAmount: ethereumNativeAmount,
      ...
      $xdaiPriceUsd: xdaiPriceUsd,
      $depositAmount: depositAmount,
      $stakedAmount: stakedAmount,
      $initialCanonicalAmount: initialCanonicalAmount,
      $result3: result3,
      $arbitrumWethAmount: arbitrumWethAmount,
      $withdrawnAmount: withdrawnAmount,
      $unstakedEthAmount: unstakedEthAmount,
      $bonderAddress: bonderAddress,
      $depositEvent: depositEvent,
      $restakedEthAmount: restakedEthAmount,
      $initialEthAmount: initialEthAmount,
      $initialMaticAmount: initialMaticAmount,
      $initialxDaiAmount: initialxDaiAmount,
      $withdrawEvent: withdrawEvent,
      $arbitrumMessengerWrapperAmount: arbitrumMessengerWrapperAmount,
      $novaBlockNumber: novaBlockNumber,
      $novaCanonicalAmount: novaCanonicalAmount,
      $novaHTokenAmount: novaHTokenAmount,
      $novaNativeAmount: novaNativeAmount,
      $baseBlockNumber: baseBlockNumber,
      $baseCanonicalAmount: baseCanonicalAmount,
      $baseHTokenAmount: baseHTokenAmount,
      $baseNativeAmount: baseNativeAmount,
      $lineaBlockNumber: lineaBlockNumber,
      $lineaCanonicalAmount: lineaCanonicalAmount,
      $lineaHTokenAmount: lineaHTokenAmount,
      $lineaNativeAmount: lineaNativeAmount,
      $polygonzkBlockNumber: polygonzkBlockNumber,
      $polygonzkCanonicalAmount: polygonzkCanonicalAmount,
      $polygonzkHTokenAmount: polygonzkHTokenAmount,
      $polygonzkNativeAmount: polygonzkNativeAmount
    })
    stmt.finalize()
  }
